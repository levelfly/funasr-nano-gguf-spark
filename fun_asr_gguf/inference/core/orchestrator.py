...
class TranscriptionOrchestrator:
    ...
    def _transcribe_long(self, audio, result, language, context, verbose, segment_size, overlap, reporter, base_offset,
        ...
        for idx, (s_s, e_s) in enumerate(segments_info):
            ...
            # Accumulate timings (use getattr for optional fields)
            for attr in ('encode', 'ctc', 'ctc_infer', 'ctc_decode', 'ctc_cast',
                         'ctc_argmax', 'ctc_loop', 'hotword_verify',
                         'prepare', 'inject', 'llm_generate', 'align'):
                src_val = getattr(d_res.timings, attr, 0) or 0
                cur_val = getattr(result.timings, attr, 0) or 0
                if hasattr(result.timings, attr):
                    setattr(result.timings, attr, cur_val + src_val)
        ...
        # Fallback: if merge returned empty text but segments have text, concatenate directly
        if not full_text and segment_results:
            full_text = "".join(r.get("text", "") for r in segment_results)
